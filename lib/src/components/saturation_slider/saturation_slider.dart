...
import 'saturation_slider_thumb.dart';
import 'saturation_slider_track.dart';
...
const double saturationTrackHeight = 10;
const double saturationThumbRadius = 16;
...
class SaturationSlider extends StatelessWidget {
  ...
  const SaturationSlider({
    ...
    this.onChangeStart,
    this.onChangeEnd,
    this.thumbRadius = saturationThumbRadius,
    this.trackHeight = saturationTrackHeight,
    this.focusNode,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return RepaintBoundary(
      child: Theme(
        data: _saturationSliderTheme(color, trackHeight, thumbRadius, context),
        child: Slider(
          value: saturation,
          divisions: 1000,
          onChanged: onChanged,
...
ThemeData _saturationSliderTheme(
  Color color,
  double trackHeight,
  double thumbRadius,
  ...
  return ThemeData.light().copyWith(
    sliderTheme: SliderThemeData(
      ...
      trackShape: SaturationSliderTrack(
        color: color,
        textDirection: Directionality.of(context),
        thumbRadius: thumbRadius,
      ),
      // The thumb uses a custom thumb that is hollow.
      thumbShape: SaturationSliderThumb(
        color: color,
